...
import type { JobInfo } from "../types/cv";
import { useAuth } from "@clerk/clerk-react";
import { useDispatch, useSelector } from "react-redux";
import type { AppDispatch, RootState } from "../store";
import { generateCV } from "../store/generateCvSlice";
import Loader from "./Loader";

const JobInfoForm: React.FC = () => {
	const { getToken } = useAuth();
	const dispatch = useDispatch<AppDispatch>();
	const { cv, loading, error } = useSelector((state: RootState) => state.cv);
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		form.append("jobtitle", jobInfo.title);
		form.append("jobDescription", jobInfo.description);
		form.append("companyInfo", jobInfo.company);
		if (jobInfo.notes) form.append("notes", jobInfo.notes);
